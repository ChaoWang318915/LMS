        ...
        <div class="card-body">
            ...
            <div class="row">
                <div class="col-md-12 form-group">
                    ...
                    {!! Form::select('media_type', ['upload' => 'Upload'],($course->mediavideo) ? $course->mediavideo->type : null,['class' => 'form-control', 'placeholder' => 'Select One','id'=>'media_type' ]) !!}
...
<script type="text/javascript">
  ...
  //ajax.open("POST", "{{ route('admin.courses.update', $course->id) }}");// http://www.developphp.com/video/JavaScript/File-Upload-Progress-Bar-Meter-Tutorial-Ajax-PHP
     document.querySelector('input[type="submit"]').addEventListener('click', function() {
		if(document.querySelector('#video_file').files.length == 0) {
			alert('Error : No file selected');
			return;
		}

		let file = document.querySelector('#video_file').files[0];
...
// 		}
		...
		data.append('file', document.querySelector('#video_file').files[0]);

		let request = new XMLHttpRequest();
		request.open('POST', '{{ route('admin.courses.update', $course->id) }}'); 
