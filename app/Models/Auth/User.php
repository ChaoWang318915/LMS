...
class User extends Authenticatable implements MessageableInterface
{
    ...
    // Commented out for update the users table.
    // protected $fillable = [
    //     'name_suffix',
    //     'first_name',
    //     'middle_name',
     ...
     * @var array
     */
    protected $appends = ['full_name','image','payment'];
    ...
    public function getPaymentAttribute(){
        $flag = 0 ;
        $payment = Payment::where('user_id',$this->id)->first();
        if(!empty($payment)) $flag = 1;
        return $flag;
    }
    ...
    {
        return $this->hasOne(UploadImage::class,'user_id');
    }
