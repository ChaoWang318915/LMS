...
class ResetPasswordController extends Controller
{
    ...
    {
        // echo  $rule = new PasswordExposed();
        // die;
        $fetch_menuItem = new BillingModel();
        $Menu_itm_data  =   $fetch_menuItem->fecth_meunItem();
        if (! $token) {
            return redirect()->route('frontend.auth.password.email',['menu_item'=>$Menu_itm_data]);
        }

        $user = $this->userRepository->findByPasswordResetToken($token);
        
        if ($user && app()->make('auth.password.broker')->tokenExists($user, $token)) {
            return view('frontend.auth.passwords.reset',['menu_item'=>$Menu_itm_data])
                ->withToken($token)
