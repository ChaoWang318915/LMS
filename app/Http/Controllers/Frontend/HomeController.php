...

{
...

    {
        ...
        if (request('page')) {
            ...
            if(request('page') == 'email-confirmed'){
            ...
            if ($page != "") {
                ...
                return view($this->path . '.pages.index', compact('page'),['menu_item'=>$Menu_itm_data]);
...

    {
        ...
        if (request('type') == 'popular') {
            ...
            $courses = Course::withoutGlobalScope('filter')->where('published', 1)->where('popular', '=', 1)->where('price','>',0)->orderBy('id', 'desc')->paginate(12);



        } else if (request('type') == 'trending') {

            $courses = Course::withoutGlobalScope('filter')->where('published', 1)->where('trending', '=', 1)->where('price','>',0)->orderBy('id', 'desc')->paginate(12);



        } else if (request('type') == 'featured') {

            $courses = Course::withoutGlobalScope('filter')->where('published', 1)->where('featured', '=', 1)->where('price','>',0)->orderBy('id', 'desc')->paginate(12);



        } else {

            $courses = Course::withoutGlobalScope('filter')->where('published', 1)->where('price','>',0)->orderBy('id', 'desc')->paginate(12);
        ...
        $fetch_menuItem = new BillingModel();
        $Menu_itm_data  =   $fetch_menuItem->fecth_meunItem();
        return view($this->path . '.search-result.courses', compact('courses', 'q', 'recent_news', 'categories'),['menu_item'=>$Menu_itm_data]);
...

    {
        ...
        $fetch_menuItem = new BillingModel();
        $Menu_itm_data  =   $fetch_menuItem->fecth_meunItem();
        return view($this->path . '.search-result.bundles', compact('bundles', 'q', 'recent_news', 'categories'),['menu_item'=>$Menu_itm_data]);
...

    {
        ...
        $fetch_menuItem = new BillingModel();
        $Menu_itm_data  =   $fetch_menuItem->fecth_meunItem();
        return view($this->path . '.search-result.blogs', compact('blogs', 'q', 'categories', 'popular_tags'),['menu_item'=>$Menu_itm_data]);
    ...
    public function viewTeachers(){
        ...
        $site_name = Cookie::get('site_name');
        $ip_address =  Cookie::get('ip_address');
        $referral_id =  Cookie::get('referral_id');
        return view('frontend/layouts/registration/registration',['menu_item'=>$Menu_itm_data,'states'=>$states,'site_name'=>$site_name,'referral_id'=>$referral_id,'ip_address'=>$ip_address]);

    }
    
    public function viewAffiliateRegister(){
        $fetch_menuItem = new BillingModel();
        $Menu_itm_data  =   $fetch_menuItem->fecth_meunItem();
        $states = State::where('country_id',231)->get();
        $site_name = Cookie::get('site_name');
        $ip_address =  Cookie::get('ip_address');
        $referral_id =  Cookie::get('referral_id');
        return view('frontend/layouts/registration/affiliate_register',['menu_item'=>$Menu_itm_data,'states'=>$states,'site_name'=>$site_name,'referral_id'=>$referral_id,'ip_address'=>$ip_address]);
    }
    
    public function viewTeachersTest(){

        //   echo  config('registration_fields');die;
        $fetch_menuItem = new BillingModel();

        $Menu_itm_data  =   $fetch_menuItem->fecth_meunItem();
        $states = State::where('country_id',231)->get();
        return view('frontend/layouts/registration/registeration_test',['menu_item'=>$Menu_itm_data,'states'=>$states]);
    ...
    public function welcome(){
        ...
        $Menu_itm_data  =   $fetch_menuItem->fecth_meunItem();
        $states = State::where('country_id',231)->get();
        return view('welcome_register',['menu_item'=>$Menu_itm_data,'states'=>$states]);
    }
    ...
    public function home(Request $request){
       ...
       $Menu_itm_data  =   $fetch_menuItem->fecth_meunItem();
        ...
        if(!empty($request->r)){
            // Set cookie 
            if(!empty($request->r)){
                Cookie::queue('referral_id', $request->r, 60*24*15);
                Cookie::queue('site_name', 'Dotcomlessons', 60*24*15);
                Cookie::queue('ip_address', $request->ip(), 60*24*15);
                $user_id = NULL;
                 
                $referral = ReferralLog::where('ip_address',$request->ip())->where('referral_id',$request->r)->first();
                if(!empty($referral)){
                    $referral_user = User::find($request->r);
                ...
                ReferralLog::create([
                    'user_id'=>$user_id,
                    'referral_id'=>$request->r,
                    'ip_address'=>$request->ip(),
                    'site_name'=>'Dotcomlessons',
                    'date'=>Carbon::today()->format('Y-m-d H:i:s'),
                ]);
            }
            
        }
        $courses = Course::where('published', 1)->orderBy('id','desc')->get()->take(5);
     
        return view('frontend/newhome',['menu_item'=>$Menu_itm_data,'courses'=>$courses]);
