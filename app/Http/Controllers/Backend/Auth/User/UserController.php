...
class UserController extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        return \DataTables::of($users)
            ...
            ->addColumn('verify_label', function ($q)  {
                return $q->verify_label;
            })
            ->addColumn('payment_verify_label', function ($q)  {
                return $q->payment_verify_label;
            })
            ...
            ->rawColumns(['confirmed_label','verify_label','payment_verify_label','roles_label','permissions_label','social_buttons','actions'])
            ->make();
    }
     ...
     * @return \Illuminate\Contracts\View\Factory|\Illuminate\View\View
     */
    public function imageVerify(ManageUserRequest $request)
    {
        if (!\Gate::allows('user_access')) {
            return abort(401);
        }
        ...
        return view('backend.auth.user.image-verify',compact('roles'))
            ->withUsers($this->userRepository->getActivePaginated(25, 'id', 'asc'));
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function getImageVerifyData(Request $request)
    {
        if($request->role &&  $request->role != ""){
            $users = User::role($request->role)->with('roles', 'permissions', 'providers')
    ...
    public function paymentVerify(ManageUserRequest $request)
    {
        if (!\Gate::allows('user_access')) {
            return abort(401);
        }
        ...
        return view('backend.auth.user.payment-verify',compact('roles'))
            ->withUsers($this->userRepository->getActivePaginated(25, 'id', 'asc'));
    }
    
    public function getPaymentVerifyData(Request $request){
        if($request->role &&  $request->role != ""){
            $users = User::role($request->role)->with('roles', 'permissions', 'providers')
            ->where('f_pay_status',0)
            ->orderBy('users.created_at', 'desc');
        }else{
            $users = User::with('roles', 'permissions', 'providers')
            ->where('f_pay_status',0)
    ...
    public function show(ManageUserRequest $request, User $user)
    {
        ...
        if(!empty($ssn[2])) $user->social_security_number = 'Xxx - xx - '.$ssn[2];
