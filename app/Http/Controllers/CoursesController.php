...
class CoursesController extends Controller
{
    ...
    public function all()
    {
        ...
        if (request('type') == 'popular') {
            $courses = Course::withoutGlobalScope('filter')->where('published', 1)->where('popular', '=', 1)->where('price','>',0)->orderBy('id', 'desc')->paginate(9);

        } else if (request('type') == 'trending') {
            $courses = Course::withoutGlobalScope('filter')->where('published', 1)->where('trending', '=', 1)->where('price','>',0)->orderBy('id', 'desc')->paginate(9);

        } else if (request('type') == 'featured') {
            $courses = Course::withoutGlobalScope('filter')->where('published', 1)->where('featured', '=', 1)->where('price','>',0)->orderBy('id', 'desc')->paginate(9);

        } else {
            $courses = Course::withoutGlobalScope('filter')->where('published', 1)->where('price','>',0)->orderBy('id', 'desc')->paginate(9);
        }
        ...
        return view( $this->path.'.courses.index', compact('courses', 'purchased_courses', 'recent_news','featured_courses','categories'),['menu_item'=>$Menu_itm_data]);
    }
    ...
    public function show($course_slug)
    {
        ...
        foreach($lessons as $lesson){
            $media = Media::where('model_type',$lesson->model_type)->where('model_id',$lesson->model_id)->first();
            if(!empty($media)) $lesson->url = $media->url;
            else $lesson->url = '';
        }
        ...
        $purchase_flag = OrderItem::whereIn('order_id',$orders)->where('item_type','App\Models\Course')->where('item_id',$course->id)->get()->count();
        
        return view( $this->path.'.courses.course', compact('course', 'purchased_course', 'purchase_flag', 'recent_news', 'course_rating', 'completed_lessons','total_ratings','is_reviewed','lessons','continue_course'),['menu_item'=>$Menu_itm_data]);
    }
    ...
    public function getByCategory(Request $request)
    {
        ...
        if ($category != "") {
            ...
            $fetch_menuItem = new BillingModel();
            $Menu_itm_data  =   $fetch_menuItem->fecth_meunItem();
            return view( $this->path.'.courses.index', compact('courses', 'category', 'recent_news','featured_courses','categories'),['menu_item'=>$Menu_itm_data]);
        }
        return abort(404);
    }
