...
//Route::get('termcond',[HomeController::class, 'term_conditions']);

Route::get('launchmodal',[HomeController::class, 'launchmodal']);
...
Route::group(['middleware' => 'auth'], function () {
    ...
    Route::get('wallet','EwalletController@showallet');
    //============= Stripe Pyment getway Routes ===============//
    Route::get('stripe', 'StripePaymentController@stripe');
    Route::post('stripe_post/{id}', 'StripePaymentController@stripePost');
    Route::post('checkMethod', 'StripePaymentController@checkMethod')->name('stripe.checkmethod');
    Route::post('make_payment', 'StripePaymentController@makePayment');
...
Route::get('registration_user', 'Frontend\HomeController@viewTeachers')->name('user_register');
Route::get('registration_affiliate', 'Frontend\HomeController@viewAffiliateRegister')->name('affiliate_register');
...
Route::group(['namespace' => 'Students', 'prefix' => 'students','middleware' => 'auth'], function () {
        //students - rewards - program
        Route::group(['prefix' => 'rewards-program'], function () {
            Route::get('/','RewardsProgramController@index')->name('students.rewards-program');
        });
        //students - payment - hisotry 
        Route::group(['prefix' => 'payment-history'], function () {
            Route::get('/','PaymentHistoryController@index')->name('students.payment-history');
        });
        //students - order - history 
        Route::group(['prefix' => 'order-history'], function () {
            Route::get('/','OrderHistoryController@index')->name('students.order-history');
        });
        //students - customer - service
        Route::group(['prefix' => 'customer-service'], function () {
            Route::get('/','CustomerServiceController@index')->name('students.customer-service');
        });
        //students - credit terms and conditions 
        Route::group(['prefix' => 'terms-condition'], function () {
            Route::get('/','TermsConditionController@index')->name('students.terms-condition');
        });
        //students - affiliate terms
        Route::group(['prefix' => 'affiliate-terms'], function () {
            Route::get('/','AffiliateTermsController@index')->name('students.affiliate-terms');
        });
        //students - account verify 
        Route::group(['prefix' => 'account-verify'], function () {
            Route::get('/','VerifyController@index')->name('students.account-verify');
            Route::post('/uploadImage','VerifyController@uploadImage')->name('students.account-verify.uploadImage');
        });
        //students - reward programming explanation
        Route::group(['prefix' => 'How-to'], function () {
            Route::get('/','DescriptionController@index')->name('students.program-description');
        });
        //students - invite student
        Route::group(['prefix' => 'invite-student'], function () {
            Route::get('/','InviteStudentController@index')->name('students.invite-student');
            Route::post('/inviteEmail','InviteStudentController@inviteEmail')->name('students.invite-student.invite_email');
        });
    });
